...
class ARAR(RecourseMethod):
    ...
    def _find_recourse(self, x: torch.Tensor, cat_feature_indices: List[int], bounds: torch.Tensor = None ):
        ...
        pbar = tqdm(range(self._outer_iters)) if self._verbose else range(self._outer_iters)
                ...
                if self._robust:
                    ...
                    if self._inner_max_pgd:
                        ...
                        for _ in range(10):
                            ...
                            loss_pertb = torch.mean(self._bce_loss(self._mlmodel.predict(x_og + pertb + delta.detach()).squeeze(),target_vec_inv))
                            loss_pertb.backward()
                            optimizer2.step()
    ...
    def get_counterfactuals(self, factuals: pd.DataFrame, bounds: torch.Tensor = None) -> pd.DataFrame:
